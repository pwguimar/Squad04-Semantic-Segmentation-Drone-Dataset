...
def _prepare_class_mappings(class_definitions, id_to_name_map):
    pass 

def _validate_masks_directory(directory_path):
    print(f"\n--- Verificando o Diretório de Máscaras ---")
    print(f"Caminho especificado: {directory_path}")

    if not os.path.isdir(directory_path):
        print(f"ERRO: O diretório '{directory_path}' NÃO FOI ENCONTRADO.")
        return [], 0

    mask_files_list = [f for f in os.listdir(directory_path) if f.lower().endswith((".png", ".jpg", ".jpeg"))]
    ...
    if total_found_masks == 0:
        ...
        print(f"Itens encontrados na pasta (primeiros 10): {os.listdir(directory_path)[:10]}")
        return [], 0

    print(f"SUCESSO: Encontrados {total_found_masks} imagens de máscara em '{directory_path}'.")
    return mask_files_list, total_found_masks

def _count_pixels_in_masks(masks_directory, mask_files_list, rgb_to_class_id_map, class_id_to_name_map, color_tolerance=10):
    class_pixel_counts_dict = {name: 0 for name in class_id_to_name_map.values()}
    ...
    for mask_filename in mask_files_list:
        ...
        for rgb_tuple, class_id in rgb_to_class_id_map.items():
            ...
            remapped_mask[matches_color_with_tolerance] = class_id
        ...
        unique_class_ids, counts = np.unique(remapped_mask, return_counts=True)
        
        for i, class_id_val in enumerate(unique_class_ids):
            if class_id_val != 255 and class_id_val in class_id_to_name_map:
                class_name = class_id_to_name_map[class_id_val]
                class_pixel_counts_dict[class_name] += counts[i]
            elif class_id_val == 255:
                pass 
            else:
                print(f"Aviso: Máscara '{mask_filename}' contém pixels com ID de classe '{class_id_val}' desconhecido (não mapeado ou 255).")
...
def _plot_distribution(sorted_class_data, percentages_data, plot_title="Distribuição de Pixels por Classe"):
    ...
    colors = ['#9B25B8', '#0EC3CE', '#7CCC00', '#FF1493', '#A9A9A9'] 
    
    plt.bar(class_names_plot, percentages_plot, color=colors[:len(class_names_plot)])
    plt.xlabel("Classe Agrupada", fontsize=12)
...
# 1 - DEFINIÇÃO DAS CLASSES MULTICLASSE (5 GRUPOS)
rgb_to_multiclass_id = {
...
# 2 - DEFINIÇÃO DO DIRETÓRIO DE MÁSCARAS MULTICLASSE
multiclass_masks_dir = "" # Substitua pelo caminho do diretório de máscaras multiclasses

if __name__ == "__main__":
    
    multiclass_mask_files, total_multiclass_masks_found = _validate_masks_directory(multiclass_masks_dir)
    
    if total_multiclass_masks_found > 0:
        multiclass_pixel_counts_result = _count_pixels_in_masks(multiclass_masks_dir, multiclass_mask_files, rgb_to_multiclass_id, multiclass_id_to_name, color_tolerance=10)
        
        min_threshold_percentage = 2.0
        sorted_multiclass_info_final, multiclass_percentages_final = _calculate_and_print_distribution(multiclass_pixel_counts_result, min_threshold_percentage, "em Pixels Multiclasse")
        
        if sorted_multiclass_info_final:
            _plot_distribution(sorted_multiclass_info_final, multiclass_percentages_final, "Distribuição de Pixels por Classe Multiclasse")